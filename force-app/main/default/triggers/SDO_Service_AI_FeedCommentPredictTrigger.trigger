trigger SDO_Service_AI_FeedCommentPredictTrigger on FeedComment (after insert) {
    if(Trigger.new.size() == 1){
        if(string.valueOf(trigger.new[0].parentID).startsWith('500')) //is case
        {
            ...
            if(postedUser.contactID == c.contactID){
                ...
                //SDO_Service_AI_SvcCloudPredictionHelper.communityUserFeedCommentSentiment(trigger.new[0].id);
            }
        }
    }
}