...
app.controller('FSLDemoToolsController', function($scope) {
  ...
  //$scope.timezonePicklist="";
	...
	$scope.numofdaysModel=$scope.getNextMonday();
  ...
  $scope.tzChanged=false;
   ...
   //multiday vars
	$scope.numofdayMultiday=$scope.getNextMonday();
	$scope.moveDatesMultiStatus='Move Dates(Multi)';
    $scope.resetMultiday='Reset Multiday data';
...
//End General
       ...
       $scope.MapInit= function(){
           console.log('in mapping initiate process:' + document.getElementById('map'));
       	...
       	searchBox.addListener('places_changed', function() {
       		...
       		if (places.length == 0) {
                ...
                console.log('map lat long'+places[0].geometry.location.lat());
                $scope.locationNewLatitude = places[0].geometry.location.lat();
    			$scope.locationNewLongitude = places[0].geometry.location.lng();
       			$scope.UpdateMap(ll);
		...
		Visualforce.remoting.Manager.invokeAction(
      		...
      		function(msg, ev){
      			...
      			if(ev.statusCode!=200){
      				...
      				console.log("Success");
      				console.log(msg);
      				$scope.finishMessage('Succesfully initiated demo data reset. Please wait for email confirmation. Click here to continue. The tool will be locked until batch job completed', true);
              $scope.GetLocations();
      			}
      			$scope.$apply();
      		});
...
$scope.moveDates=function(){
	$scope.timeButtonStatus="Processing...";
	   	Visualforce.remoting.Manager.invokeAction(
      		RemoteActions.MoveDates,$scope.numofdaysModel.getFullYear(),$scope.numofdaysModel.getMonth()+1,$scope.numofdaysModel.getDate(),
      		function(msg, ev){
      			$scope.working=false;
      			...
      			if(ev.statusCode!=200){
      				...
      				console.log("Success");
      				console.log(msg);
      				$scope.finishMessage('Succesfully initiated date mover. Please wait for email confirmation. Click here to continue. The tool will be locked until batch job completed', true);
      			}
      			$scope.$apply();
      		});
}
      ...
      Visualforce.remoting.Manager.invokeAction(
          ...
          function(msg, ev){
            ...
            if(ev.statusCode!=200){
              ...
              console.log("Success");
              console.log(msg);
              $scope.finishMessage('Succesfully initiated time zone mover. Please wait for email confirmation. Click here to continue. The tool will be locked until batch job completed', true);
            }
            $scope.$apply();
          });
}
...
//$scope.numofdayMultiday=0;
//$scope.moveDatesMultiStatus='Move Dates(Multi)';
    
$scope.moveMultidayAppointment= function(){
	$scope.timeButtonStatus="Processing...";
	   	Visualforce.remoting.Manager.invokeAction(
      		RemoteActions.moveMultiday ,$scope.numofdayMultiday.getFullYear(),$scope.numofdayMultiday.getMonth()+1,$scope.numofdayMultiday.getDate(),
      		function(msg, ev){
      			$scope.working=false;
      			$scope.timeButtonStatus="Move Dates";
      			$scope.numofdayMultiday = 0;
      			...
      			if(ev.statusCode!=200){
      				...
      				console.log("Success");
      				console.log(msg);
      				$scope.finishMessage('Succesfully initiated date mover for multiday service crew appointments. Please wait for email confirmation. The tool will be locked until batch job completed', true);
      			}
      			$scope.$apply();
      		});
}
