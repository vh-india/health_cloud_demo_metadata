public class SDO_Service_EinsteinCaseClassForEmail {
    public static List<SDO_Service_Einstein_Agent_Helper__c> helperConfig;
    
    @InvocableMethod(label='Create Case Classifications for Email' callout=true)
    public static List<String> createRecommendations(List<Case> cases){
        Case c = cases[0];
        ...
        List<SDO_Service_Einstein_Agent_Helper__c> helperConfig = [Select Id,Field__c,Comparator__c,Value__c,Confidence__c,
                                                                   Recommendation_Field__c,Recommendation_Value__c,
                                                                   Recommended_Article_Score__c
                                                                   From SDO_Service_Einstein_Agent_Helper__c 
        ...
        if(predictionFieldsMap.size() > 0){
            ...
            for(String fieldName: predictionFieldsMap.keySet()){
                if(fieldName == ''){
                    predictionFieldsMap.get(fieldName);
                }
        ...
        return new List<String>{classificationResult};
    }
    
    public static void createRecommendations(Case c){
        DateTime now = DateTime.now();
        String currentTime = now.formatGmt('yyyy-MM-dd\'T\'HH:mm:ss\'Z\'');
        Map<String,List<cc_AiInsightValue>> predictionFieldsMap = new Map<String,List<cc_AiInsightValue>>();
