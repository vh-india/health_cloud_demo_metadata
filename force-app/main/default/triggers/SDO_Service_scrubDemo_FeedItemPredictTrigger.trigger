trigger SDO_Service_scrubDemo_FeedItemPredictTrigger on FeedItem (after insert) {
    if(Trigger.new.size() == 1){
        if(string.valueOf(trigger.new[0].parentID).startsWith('500')) //is case
        {
            ...
            if(c != null && c.size() > 0 && postedUser.contactID == c[0].contactID){
                ...
                //SDO_Service_AI_SvcCloudPredictionHelper.communityUserFeedItemSentiment(trigger.new[0].id);
            }
        }
    }
}